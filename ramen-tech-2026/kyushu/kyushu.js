...
(() => {
  ...
  const sourceLink = (url,label,prominent=false) => `<a${prominent?' class="source-publisher"':''} href="${esc(url)}" target="_blank" rel="noopener noreferrer">${esc(label)} ↗</a>`;
  function filter() {
  ...
  function openEvent(id,button) {
    ...
    document.getElementById('dialog-inner').innerHTML=`<figure class="dialog-media"><img class="dialog-photo" src="${esc(e.image.url)}" alt="${esc(e.image.alt)}"><figcaption class="image-disclosure"><strong>油絵風・AI生成イメージ</strong>実際の会場・出演者・作品・商品・景観の写真ではありません。</figcaption></figure><div class="dialog-content"><p class="section-kicker">${esc(e.areaLabel)} / ${esc(e.categoryLabel)}</p><h2 id="dialog-title">${esc(e.hook)}</h2><p class="dialog-name">${esc(e.name)}</p><p class="dialog-description">${esc(e.description)}</p><dl class="detail-grid"><div><dt>開催日</dt><dd>${esc(e.dateLabel)}</dd></div><div><dt>時間・参加条件</dt><dd>${esc(e.admission)}</dd></div><div class="detail-wide"><dt>会場</dt><dd>${esc(e.venue)}</dd></div><div class="detail-wide"><dt>福岡からの行き方</dt><dd>${esc(e.access)}</dd></div></dl><div class="dialog-links"><a href="${esc(e.source)}" target="_blank" rel="noopener noreferrer">${esc(e.sourceLabel || '公式情報・参加方法を見る')} ↗</a><a href="${esc(e.map)}" target="_blank" rel="noopener noreferrer">会場の地図・経路 ↗</a></div>${e.note?`<p class="dialog-notice">${esc(e.note)}</p>`:''}<section class="dialog-sources" aria-label="開催情報の出典"><h3>開催情報の掲載元 <small>${esc(sourceKind)}</small></h3><p>${sourceLink(e.source,e.sourceName,true)}</p>${e.additionalSource?`<p>${sourceLink(e.additionalSource,e.additionalSourceName)}</p>`:''}<p>開催情報の確認日：${esc(e.checked)}<br>最新の開催状況・料金・予約条件は主催者の案内をご確認ください。</p></section><p class="dialog-credit">${esc(e.image.credit)} ／ 画像制作日：${esc(e.image.created)}<br>画像制作日は開催情報の再確認日ではありません。</p></div>`;
