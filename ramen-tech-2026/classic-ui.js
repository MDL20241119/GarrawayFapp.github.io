...
(()=>{
  ...
  if(!hero||hero.dataset.classic==='20260912k4')return;
  const initialHash=location.hash;
  hero.dataset.classic='20260912k4';hero.classList.add('classic-hero');
  ...
  const jump=()=>{if(document.querySelector('#screen-explore')?.hidden)return;window.scrollTo({top:Math.max(0,window.scrollY+title.getBoundingClientRect().top-(document.querySelector('.site-header')?.getBoundingClientRect().height||65)),behavior:'instant'});};
  const fallbackMap=()=>{
