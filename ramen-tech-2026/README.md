...
- 公式登録と公式タイムテーブルを毎時07分に取得開始。GitHub Actionsの混雑・配信処理で遅延する場合があります。
...
- 最終取得成功から3時間を超えると「更新が遅れています」と表示します。「常時リアルタイム」を保証するものではありません。
- `tools/ramen_reconcile.py` は確認済みの重複だけを同一企画にまとめ、旧IDを保存済み予定向けに残します。
...
`sync-status.json` に実際の実行時刻、最終成功時刻、ソース別成否、件数と変更内容を保存。ページ上部で確認できます。取得失敗・異常な件数減少・形式変更があれば、該当ソースの前回データを保持し、成功したふりをせず警告します。失敗が続いて最終成功から3時間を超えた場合も警告します。
