...
def prepare(root):
    ...
    js = js.replace('function detail(id,day){const e=', 'function detail(id,day){id=window.RamenIdentity.canonicalId(D.events,id);const e=')
    js = js.replace('このガイドは配信時点の保存データを表示し、閲覧中に自動取得は行いません。', '公式登録・公式タイムテーブルを毎朝6時に確認し、閲覧中も5分ごとに配信データを確認します。取得・配信には時間差があります。')
    js = js.replace('このプレビューは保存データを使い、自動取得処理を実行しません。', '公式登録・公式タイムテーブルを毎朝6時に確認し、閲覧中も5分ごとに配信データを確認します。取得・配信には時間差があります。')
    old='D=next;window.RAMEN_CATALOG=next;rebuildIndexes();renderCurrent();'
    new="D=next;window.RAMEN_CATALOG=next;rebuildIndexes();model=hydrate(model);if(demo)userModel=hydrate(userModel);else userModel=model;allDays=[...new Set(D.events.filter(e=>!e.duplicateOf).flatMap(e=>e.dates))].sort();counts();renderCurrent();"
