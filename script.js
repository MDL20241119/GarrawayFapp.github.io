...
if(newsSection){
 ...
 const status=item=>new Date(item.date+'T00:00:00')>=today?'UPCOMING':'ARCHIVE';
 const pretty=s=>s.replaceAll('-','.');
 const head=newsSection.querySelector('.news-live-head');
 if(head){
  const desc=head.querySelector('p:not(.kicker)');if(desc)desc.textContent='Garraway Fで開催されるイベントや、日々生まれている挑戦をお届けします。';
 ...
 if(frame){
  frame.id='news-list';frame.className='news-native-wrap';
  frame.innerHTML='<nav class="news-filters" aria-label="ニュース＆イベント絞り込み"><button class="is-active" data-filter="ALL">ALL</button><button data-filter="UPCOMING">UPCOMING</button><button data-filter="EVENT">EVENT</button><button data-filter="NEWS">NEWS</button><button data-filter="ARCHIVE">ARCHIVE</button></nav><div class="news-native-grid"></div>';
  const grid=frame.querySelector('.news-native-grid');
  const render=(filter='ALL')=>{
   const items=newsEvents.filter(item=>filter==='ALL'||item.type===filter||status(item)===filter);
   grid.innerHTML=items.length?items.map((item,i)=>`<article class="news-card"><div class="news-meta"><span>${String(i+1).padStart(2,'0')}</span><span>${item.type}</span></div><time datetime="${item.date}">${pretty(item.date)}</time><h3>${item.title}</h3><div class="news-card-foot"><span>${status(item)}</span><span>↗</span></div></article>`).join(''):'<p class="news-empty">該当する情報はありません。</p>';
  };
  render();
  frame.querySelectorAll('.news-filters button').forEach(btn=>btn.addEventListener('click',()=>{frame.querySelectorAll('button').forEach(b=>b.classList.remove('is-active'));btn.classList.add('is-active');render(btn.dataset.filter)}));
 }
